            ...
            <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:p-0">
                ...
                <div
                            ...
                            @foreach ($templates as $template)
                                <div wire:click="selectTemplate({{ $template->id }})"
                                    ...
                                    @if ($template->category)
                                        <span
                                            class="inline-block px-2 py-1 text-xs font-medium text-indigo-700 bg-indigo-100 rounded mb-2">
                                            {{ ucfirst(str_replace('_', ' ', $template->category)) }}
                                        </span>
                                    @endif
                                    @if ($template->description)
                                        <p class="text-sm text-gray-600">{{ Str::limit($template->description, 80) }}
                                        </p>
                                    @endif
                                </div>
                            @endforeach
                                ...
                                <div class="space-y-4">
                                    ...
                                    @if ($previewData['category'])
                                        <div>
                                            ...
                                            <p class="mt-1 text-gray-900">
                                                {{ ucfirst(str_replace('_', ' ', $previewData['category'])) }}</p>
                                        </div>
                                    @endif
                                    ...
                                    <div>
                                        <label class="block text-sm font-medium text-gray-700">Terms &
                                            Conditions</label>
                                        <p class="mt-1 text-gray-900">
                                            {{ $previewData['has_terms'] ? 'Included' : 'Not included' }}</p>
                                    </div>
                                    ...
                                    <div class="pt-4 border-t">
                                        <div class="bg-yellow-50 border border-yellow-200 rounded-lg p-3">
                                            <p class="text-sm text-yellow-800">
                                                <strong>Warning:</strong> Applying this template will update the quote
                                                details and add template items. Existing items will not be removed.
                                            </p>
                                        </div>
                                    </div>
                                </div>
                    ...
                    <div class="mt-6 flex gap-3">
                        <button wire:click="applyTemplate" :disabled="!$selectedTemplateId"
