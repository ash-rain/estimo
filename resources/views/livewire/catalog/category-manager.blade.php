                        ...
                        <select id="parent_id" wire:model="parent_id"
                            ...
                            @foreach ($parentCategories as $category)
                                ...
                                @foreach ($category->children as $child)
                                    <option value="{{ $child->id }}">{{ $category->name }} > {{ $child->name }}
                                    </option>
                                @endforeach
                            @endforeach
                        </select>
            ...
            <div class="border border-gray-200 rounded-lg p-4">
                <div class="flex items-center justify-between">
                    <div class="flex-1">
                        <div class="flex items-center gap-3">
                            ...
                            @if (!$category->is_active)
                                <span
                                    class="px-2 py-1 text-xs font-medium text-gray-600 bg-gray-100 rounded">Inactive</span>
                            @endif
                        </div>
                    ...
                    <div class="flex items-center gap-2">
                        <button wire:click="moveUp({{ $category->id }})" class="p-1 text-gray-400 hover:text-gray-600"
                            title="Move up">
                        ...
                        <button wire:click="toggleActive({{ $category->id }})"
                            ...
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M{{ $category->is_active ? '18.364 18.364A9 9 0 005.636 5.636m12.728 12.728A9 9 0 015.636 5.636m12.728 12.728L5.636 5.636' : '9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z' }}" />
                            </svg>
                        </button>
                            ...
                            <div class="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                                <div class="flex-1">
                                    <div class="flex items-center gap-2">
                                        ...
                                        @if (!$child->is_active)
                                            <span
                                                class="px-2 py-0.5 text-xs font-medium text-gray-600 bg-gray-200 rounded">Inactive</span>
                                        @endif
                                    </div>
                                ...
                                <div class="flex items-center gap-2">
                                    ...
                                    <button wire:click="toggleActive({{ $child->id }})"
                                        ...
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor"
                                            viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                d="M{{ $child->is_active ? '18.364 18.364A9 9 0 005.636 5.636m12.728 12.728A9 9 0 015.636 5.636m12.728 12.728L5.636 5.636' : '9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z' }}" />
                                        </svg>
                                    </button>
