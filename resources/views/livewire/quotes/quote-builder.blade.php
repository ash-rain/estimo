    ...
    <form wire:submit.prevent="save">
        ...
        <div class="bg-white rounded-lg shadow-sm p-6 mb-6">
            ...
            <div class="mt-4 flex gap-2">
                ...
                @if($quote->exists && $quote->status === 'draft')
                    <button
                        type="button"
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50">
                            <tr>
                                ...
                                <th class="px-3 py-2 text-right text-xs font-medium text-gray-500 uppercase w-24">Disc %</th>
                                ...
                                <tr>
                                    ...
                                    <td class="px-3 py-3 text-right">
                                        <input
                                            ...
                                            min="0"
                                            max="100"
                                            class="w-20 text-right text-sm rounded border-gray-300"
                                        >
                                    </td>
            ...
            <div class="bg-white rounded-lg shadow-sm p-6 mb-6">
                <div class="max-w-md ml-auto">
                    <div class="grid grid-cols-2 gap-4 mb-4">
                        ...
                        <div>
                            <label for="discount_rate" class="block text-sm font-medium text-gray-700 mb-1">Discount (%)</label>
                            <input
                                type="number"
                                id="discount_rate"
                                wire:model.live="discount_rate"
                                step="0.01"
                                min="0"
                                max="100"
                                class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500"
                    ...
                    <div class="border-t border-gray-200 pt-4 space-y-2">
                        ...
                        @if($quote->discount_amount > 0)
                            <div class="flex justify-between text-sm">
                                ...
                                <span class="font-medium text-red-600">-{{ number_format($quote->discount_amount, 2) }}</span>
                            </div>
                        @endif
            ...
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <textarea
                        ...
                        rows="2"
                        class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500"
                        placeholder="Thank you message, contact info, etc."
                    ></textarea>
                </div>
            </div>
        </div>
    ...
    @if($showAddItemModal)
        <div class="fixed inset-0 z-50 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
            <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                <div class="fixed inset-0 bg-gray-500 bg-opacity-75 transition-opacity" wire:click="$set('showAddItemModal', false)"></div>
                        ...
                        <div class="mb-4">
                            ...
                            <div class="max-h-64 overflow-y-auto border border-gray-200 rounded-md">
                                @foreach($catalogItems as $catalogItem)
                                    <button
                                        type="button"
                                        wire:click="addItemFromCatalog({{ $catalogItem->id }})"
                                        class="w-full text-left px-4 py-3 hover:bg-gray-50 border-b border-gray-100 last:border-0"
                                    >
                                        <div class="flex justify-between items-center">
                                            <div>
                                                <div class="text-sm font-medium text-gray-900">{{ $catalogItem->name }}</div>
                                                <div class="text-xs text-gray-500">{{ $catalogItem->category?->name ?? 'Uncategorized' }} | {{ $catalogItem->unit_type }}</div>
                                            </div>
                                            <div class="text-sm font-medium text-gray-700">
                                                {{ $catalogItem->currency }} {{ number_format($catalogItem->selling_price, 2) }}
                                            </div>
                                        </div>
                                    </button>
                                @endforeach
